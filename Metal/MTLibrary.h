...
#pragma once
// NOTE: do not include "MTDevice.h" here — MTDevice depends on MTLibrary, not
// the other way around. Including it created an MTDevice<->MTLibrary include
// cycle. MTLibrary only needs the opaque handles (via MTForwardDecls).
...
MT_INLINE MTLibrary mt_device_create_default_library(MTDevice device) {
    return MT_MSG_SEND(void*, device, MT_SEL("newDefaultLibrary"));
}

MT_INLINE MTLibrary mt_device_create_library_with_url(MTDevice device, MTURL url, MTError * error) {
    return MT_MSG_SEND_2(void*, device, MT_SEL("newLibraryWithURL:error:"), id, url, MTError *, error);
}

MT_INLINE MTLibrary mt_device_create_library_with_data(MTDevice device, dispatch_data_t data, MTError * error) {
    return MT_MSG_SEND_2(void*, device, MT_SEL("newLibraryWithData:error:"), dispatch_data_t, data, MTError *, error);
}
...
MT_INLINE MTLibrary mt_device_create_library_with_source(
    ...
    return MT_MSG_SEND_3(void*, device, MT_SEL("newLibraryWithSource:options:error:"), id, source, id, options, MTError *, error);
}
...
MT_INLINE void mt_device_create_library_with_source_async(
    ...
    void (^handler)(id, MTError) = ^(id lib, MTError err) {
        callback(userdata, (MTLibrary)lib, err);
    };

    MsgSendFn fn = (MsgSendFn)objc_msgSend;
    fn((id)device, sel, source, options, handler);
}

MT_INLINE MTFunction mt_library_create_function(MTLibrary library, MTString name) {
    return MT_MSG_SEND_1(void*, library, MT_SEL("newFunctionWithName:"), id, name);
}
