...
#pragma once
#include "MTForwardDecls.h"
#include "MTFoundation/defines.h"

#define MT_PACKED __attribute__((packed))

// Legacy macros (deprecated - use MT_ALLOC_INIT instead)
#define NSCLASS_NEW(name) MT_MSG_SEND_CLASS(void*, MT_CLASS(name), MT_SEL("new"))
#define NSCLASS_ALLOC_INIT(name) MT_ALLOC_INIT(name)
#define NSCLASS_ALLOC(name) MT_MSG_SEND_CLASS(void*, MT_CLASS(name), MT_SEL("alloc"))
