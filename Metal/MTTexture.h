...
MT_INLINE MTSharedTextureHandle mt_texture_new_shared_texture_handle(MTTexture texture) {
    return MT_MSG_SEND(void*, texture, MT_SEL("newSharedTextureHandle"));
}

MT_INLINE MTDevice mt_shared_texture_handle_get_device(MTSharedTextureHandle handle) {
    return MT_MSG_SEND(void*, handle, MT_SEL("device"));
}
...
MT_INLINE MTTextureDescriptor mt_texture_descriptor_create(MTPixelFormat pixel_format, NSUInteger width, NSUInteger height, bool mipmapped) {
    ...
    // Autoreleased convenience factory; retain so _create means "+1 owned".
    return MT_RETAIN(((void* (*)(Class, SEL, MTPixelFormat, NSUInteger, NSUInteger, BOOL))objc_msgSend)(
...
MT_INLINE MTTextureDescriptor mt_texture_cube_descriptor_create(MTPixelFormat pixel_format, NSUInteger size, bool mipmapped) {
    return MT_RETAIN(((void* (*)(Class, SEL, MTPixelFormat, NSUInteger, BOOL))objc_msgSend)(MT_CLASS("MTLTextureDescriptor"), MT_SEL("textureCubeDescriptorWithPixelFormat:size:mipmapped:"), pixel_format, size, mipmapped));
}
...
MT_INLINE MTTextureDescriptor mt_texture_buffer_descriptor_create(MTPixelFormat pixel_format, NSUInteger width, MTResourceOptions resource_options, MTTextureUsage usage) {
    return MT_RETAIN(((void* (*)(Class, SEL, MTPixelFormat, NSUInteger, MTResourceOptions, MTTextureUsage))objc_msgSend)(MT_CLASS("MTLTextureDescriptor"), MT_SEL("textureBufferDescriptorWithPixelFormat:width:resourceOptions:usage:"), pixel_format, width, resource_options, usage));
}

MT_INLINE MTTexture mt_texture_new_texture_view_with_format(MTTexture texture, MTPixelFormat pixel_format) {
    return MT_MSG_SEND_1(void*, texture, MT_SEL("newTextureViewWithPixelFormat:"), MTPixelFormat, pixel_format);
}
...
MT_INLINE MTTexture mt_texture_new_texture_view_with_full_desc(MTTexture texture, MTPixelFormat pixel_format, MTTextureType texture_type, MTRange levels, MTRange slices) {
    return ((void* (*)(id, SEL, MTPixelFormat, MTTextureType, MTRange, MTRange))objc_msgSend)((id)texture, MT_SEL("newTextureViewWithPixelFormat:textureType:levels:slices:"), pixel_format, texture_type, levels, slices);
}
...
MT_INLINE MTTexture mt_texture_get_remote_storage_texture(MTTexture texture) {
    return MT_MSG_SEND(void*, texture, MT_SEL("remoteStorageTexture"));
}
...
MT_INLINE MTDevice mt_texture_get_device(MTTexture texture) {
    return MT_MSG_SEND(void*, texture, MT_SEL("device"));
}
...
MT_INLINE MTResourceOptions mt_texture_get_resource_options(MTTexture texture) {
...
MT_INLINE MTHeap mt_texture_get_heap(MTTexture texture) {
    return MT_MSG_SEND(void*, texture, MT_SEL("heap"));
}

MT_INLINE uintptr_t mt_texture_get_heap_offset(MTTexture texture) {
    return MT_MSG_SEND(uintptr_t, texture, MT_SEL("heapOffset"));
}

MT_INLINE uintptr_t mt_texture_get_allocated_size(MTTexture texture) {
