...
MT_INLINE MTIOCommandBuffer mt_io_command_queue_get_command_buffer(MTIOCommandQueue queue) {
    return MT_MSG_SEND(void*, queue, MT_SEL("commandBuffer"));
}

MT_INLINE MTIOCommandBuffer mt_io_command_queue_command_buffer_unretained(MTIOCommandQueue queue) {
    return MT_MSG_SEND(void*, queue, MT_SEL("commandBufferWithUnretainedReferences"));
}

MT_INLINE MTString mt_io_command_queue_get_label(MTIOCommandQueue queue) {
    return MT_MSG_SEND(void*, queue, MT_SEL("label"));
}
...
MT_INLINE MTString mt_io_file_handle_get_label(MTIOFileHandle handle) {
    return MT_MSG_SEND(void*, handle, MT_SEL("label"));
}
