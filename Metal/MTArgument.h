...
MT_INLINE MTString mt_binding_get_name(MTBinding binding) {
    return MT_MSG_SEND(void*, binding, MT_SEL("name"));
}
...
MT_INLINE void* mt_buffer_binding_get_struct_type(MTBufferBinding binding) {
    return MT_MSG_SEND(void*, binding, MT_SEL("bufferStructType"));
}

MT_INLINE void* mt_buffer_binding_get_pointer_type(MTBufferBinding binding) {
    return MT_MSG_SEND(void*, binding, MT_SEL("bufferPointerType"));
}
...
MT_INLINE MTString mt_struct_member_get_name(MTStructMember member) {
    return MT_MSG_SEND(void*, member, MT_SEL("name"));
}
...
MT_INLINE MTArray mt_struct_type_get_members(MTStructType type) {
    return MT_MSG_SEND(void*, type, MT_SEL("members"));
}

MT_INLINE MTStructMember mt_struct_type_get_member_by_name(MTStructType type, MTString name) {
    return MT_MSG_SEND_1(void*, type, MT_SEL("memberByName:"), id, name);
}
...
MT_INLINE MTStructMember mt_array_get_object_at_index(MTArray array, NSUInteger index) {
    return MT_MSG_SEND_1(void*, array, MT_SEL("objectAtIndex:"), NSUInteger, index);
}

// Typed accessor for the binding arrays returned by pipeline reflection
// (mt_*_pipeline_reflection_get_*_bindings). Use with mt_array_get_count.
MT_INLINE MTBinding mt_array_get_binding_at_index(MTArray array, NSUInteger index) {
    return MT_MSG_SEND_1(void*, array, MT_SEL("objectAtIndex:"), NSUInteger, index);
}
