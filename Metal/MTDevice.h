...
MT_INLINE const char* mt_device_get_name(MTDevice device) {
    ...
    id nsStr = MT_MSG_SEND(void*, device, MT_SEL("name"));
...
MT_INLINE MTBuffer mt_device_create_buffer_with_bytes(MTDevice device, void* ptr, uintptr_t length, MTResourceOptions options) {
    return MT_MSG_SEND_3(void*, device, MT_SEL("newBufferWithBytes:length:options:"), void*, ptr, uintptr_t, length, MTResourceOptions, options);
}

MT_INLINE MTBuffer mt_device_create_buffer_with_length(MTDevice device, uintptr_t length, MTResourceOptions options) {
    return MT_MSG_SEND_2(void*, device, MT_SEL("newBufferWithLength:options:"), uintptr_t, length, MTResourceOptions, options);
}
...
MT_INLINE MTIndirectCommandBuffer mt_device_new_indirect_command_buffer(MTDevice device, MTIndirectCommandBufferDescriptor descriptor, unsigned long maxCommandCount, MTResourceOptions options) {
    return MT_MSG_SEND_3(void*, device, MT_SEL("newIndirectCommandBufferWithDescriptor:maxCommandCount:options:"), id, descriptor, unsigned long, maxCommandCount, MTResourceOptions, options);
}
...
MT_INLINE MTRenderPipelineState mt_device_create_render_pipeline_state(MTDevice device, MTRenderPipelineDescriptor render_desc, MTError * error) {
    return MT_MSG_SEND_2(void*, device, MT_SEL("newRenderPipelineStateWithDescriptor:error:"), id, render_desc, MTError *, error);
}

MT_INLINE MTComputePipelineState mt_device_create_compute_pipeline_state_with_function(MTDevice device, MTFunction function, MTError * error) {
    return MT_MSG_SEND_2(void*, device, MT_SEL("newComputePipelineStateWithFunction:error:"), id, function, MTError *, error);
}

MT_INLINE MTComputePipelineState mt_device_create_compute_pipeline_state_with_descriptor(MTDevice device, MTComputePipelineDescriptor descriptor, MTError * error) {
    return MT_MSG_SEND_4(void*, device, MT_SEL("newComputePipelineStateWithDescriptor:options:reflection:error:"), id, descriptor, NSUInteger, 0, id, NULL, MTError *, error);
}

MT_INLINE void mt_device_new_render_pipeline_state_async(MTDevice device, MTRenderPipelineDescriptor descriptor, MTNewRenderPipelineStateCallback callback, void *userdata) {
    ...
    fn((id)device, sel, (id)descriptor, handler);
}

MT_INLINE MTRenderPipelineState mt_new_render_pipeline_state_with_reflection(MTDevice device, MTRenderPipelineDescriptor desc, MTPipelineOption options, MTRenderPipelineReflection **out_reflection, MTError *out_error) {
    return MT_MSG_SEND_4(void*, device, MT_SEL("newRenderPipelineStateWithDescriptor:options:reflection:error:"), id, desc, MTPipelineOption, options, MTRenderPipelineReflection**, out_reflection, MTError *, out_error);
}

MT_INLINE void mt_new_render_pipeline_state_with_reflection_async(MTDevice device, MTRenderPipelineDescriptor desc, MTPipelineOption options, render_pipeline_with_reflection_callback_t callback, void *userdata) {
    ...
    MsgSendFn fn = (MsgSendFn)objc_msgSend;
    fn((id)device, sel, (id)desc, options, handler);
}
...
MT_INLINE MTTexture mt_device_create_texture_with_descriptor(MTDevice device, MTTextureDescriptor texture_desc) {
    return MT_MSG_SEND_1(void*, device, MT_SEL("newTextureWithDescriptor:"), id, texture_desc);
}

MT_INLINE MTTexture mt_device_create_shared_texture_with_descriptor(MTDevice device, MTTextureDescriptor texture_desc) {
    return MT_MSG_SEND_1(void*, device, MT_SEL("newSharedTextureWithDescriptor:"), id, texture_desc);
}

MT_INLINE MTSamplerState mt_device_create_sampler_state(MTDevice device, MTSamplerDescriptor sampler_desc) {
    return MT_MSG_SEND_1(void*, device, MT_SEL("newSamplerStateWithDescriptor:"), id, sampler_desc);
}

MT_INLINE MTDepthStencilState mt_device_create_depth_stencil_state(MTDevice device, MTDepthStencilDescriptor desc) {
    return MT_MSG_SEND_1(void*, device, MT_SEL("newDepthStencilStateWithDescriptor:"), id, desc);
}
...
MT_INLINE MTHeap mt_device_create_heap_with_descriptor(MTDevice device, MTHeapDescriptor descriptor) {
    return MT_MSG_SEND_1(void*, device, MT_SEL("newHeapWithDescriptor:"), id, descriptor);
}
...
MT_INLINE MTIOFileHandle mt_device_new_io_file_handle(MTDevice device, MTURL url, MTError * outError) {
    return MT_MSG_SEND_2(void*, device, MT_SEL("newIOFileHandleWithURL:error:"), id, url, MTError *, outError);
}
...
MT_INLINE MTIOFileHandle mt_device_new_io_file_handle_with_compression(MTDevice device, MTURL url, MTIOCompressionMethod compression, MTError * outError) {
    return MT_MSG_SEND_3(void*, device, MT_SEL("newIOFileHandleWithURL:compressionMethod:error:"), id, url, unsigned long, compression, MTError *, outError);
}
...
MT_INLINE MTIOCommandQueue mt_device_new_io_command_queue(MTDevice device, MTIOCommandQueueDescriptor desc, MTError * outError) {
    return MT_MSG_SEND_2(void*, device, MT_SEL("newIOCommandQueueWithDescriptor:error:"), id, desc, MTError *, outError);
}
...
MT_INLINE MTBinaryArchive mt_device_new_binary_archive(MTDevice device, MTBinaryArchiveDescriptor desc, MTError * outError) {
    return MT_MSG_SEND_2(void*, device, MT_SEL("newBinaryArchiveWithDescriptor:error:"), id, desc, MTError *, outError);
}

// ============================================================================
// Synchronization primitives (all +1 owned — release with mt_release)
// ============================================================================

// Lightweight intra-command-buffer fence for ordering untracked resources.
MT_INLINE MTFence mt_device_create_fence(MTDevice device) {
    return MT_MSG_SEND(void*, device, MT_SEL("newFence"));
}

// GPU-only event for cross-queue ordering on the same device.
MT_INLINE MTEvent mt_device_create_event(MTDevice device) {
    return MT_MSG_SEND(void*, device, MT_SEL("newEvent"));
}

// Event that can also be signalled/waited from the CPU (and across processes).
MT_INLINE MTSharedEvent mt_device_create_shared_event(MTDevice device) {
    return MT_MSG_SEND(void*, device, MT_SEL("newSharedEvent"));
}
