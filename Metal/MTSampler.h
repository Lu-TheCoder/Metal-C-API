...
MT_INLINE MTSamplerDescriptor mt_sampler_descriptor_create(void) {
...
MT_INLINE void mt_sampler_descriptor_set_mip_filter(MTSamplerDescriptor desc, NSUInteger mip_filter) {
...
// Required for the sampler's gpuResourceID to be usable in an argument
// buffer / Metal 4 argument table (mt4_argument_table_set_sampler_state).
MT_INLINE void mt_sampler_descriptor_set_support_argument_buffers(MTSamplerDescriptor desc, bool support) {
...
MT_INLINE void mt_sampler_set_label(MTSamplerDescriptor desc, const char* label) {
    id str = MT_MSG_SEND_CLASS_1(void*, MT_CLASS("NSString"), MT_SEL("stringWithUTF8String:"), const char*, label);
...
MT_INLINE const char* mt_sampler_state_get_label(MTSamplerState samplerState) {
    id nsLabel = MT_MSG_SEND(void*, samplerState, MT_SEL("label"));
...
MT_INLINE MTDevice mt_sampler_state_get_device(MTSamplerState samplerState) {
    return MT_MSG_SEND(void*, samplerState, MT_SEL("device"));
}
