...
MT_INLINE void mt_io_command_buffer_add_completed_handler(MTIOCommandBuffer cmdBuf, MTIOCommandCallback callback, void* userData) {
    void (^handlerBlock)(id) = ^(id completed_buf) {
        (void)completed_buf;
        callback(userData);
    };
    MT_MSG_SEND_1(void, cmdBuf, MT_SEL("addCompletedHandler:"), void (^)(id), handlerBlock);
}
...
MT_INLINE void mt_io_command_buffer_load_texture(MTIOCommandBuffer cmdBuf, MTTexture texture, NSUInteger slice, NSUInteger level, MTSize size, NSUInteger sourceBytesPerRow, NSUInteger sourceBytesPerImage, MTOrigin destinationOrigin, MTIOFileHandle sourceHandle, NSUInteger sourceHandleOffset) {
    ((void (*)(id, SEL, id, NSUInteger, NSUInteger, MTSize, NSUInteger, NSUInteger, MTOrigin, id, NSUInteger))objc_msgSend)(
        (id)cmdBuf,
        MT_SEL("loadTexture:slice:level:size:sourceBytesPerRow:sourceBytesPerImage:destinationOrigin:sourceHandle:sourceHandleOffset:"),
        (id)texture,
        slice,
        level,
        size,
...
MT_INLINE MTString mt_io_command_buffer_get_label(MTIOCommandBuffer cmdBuf) {
    return MT_MSG_SEND(void*, cmdBuf, MT_SEL("label"));
}
...
MT_INLINE MTError mt_io_command_buffer_get_error(MTIOCommandBuffer cmdBuf) {
    return MT_MSG_SEND(void*, cmdBuf, MT_SEL("error"));
}
