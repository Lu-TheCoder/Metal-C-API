...
MT_INLINE MTURL mt_url_init_with_path(MTString path) {
    id alloc = MT_MSG_SEND_CLASS(void*, MT_CLASS("NSURL"), MT_SEL("alloc"));
    return MT_MSG_SEND_1(void*, alloc, MT_SEL("initFileURLWithPath:"), id, path);
}

MT_INLINE MTURL mt_url_init_with_string(MTString urlString) {
    return MT_MSG_SEND_CLASS_1(void*, MT_CLASS("NSURL"), MT_SEL("URLWithString:"), id, urlString);
}

/// Get absoluteString from NSURL
MT_INLINE const char* mt_url_get_absolute_string(MTURL url) {
    MTString str = MT_MSG_SEND(void*, url, MT_SEL("absoluteString"));
    return mt_string_to_utf8(str);
}

/// Get path from NSURL
MT_INLINE const char* mt_url_get_path(MTURL url) {
    MTString pathStr = MT_MSG_SEND(void*, url, MT_SEL("path"));
